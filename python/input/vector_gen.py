...
def convert_hex_to_unified_header(output_file="input_data.h"):
    ...
    with open(output_file, 'w') as f_out:
        # 헤더 가드 작성
        f_out.write("#ifndef INPUT_DATA_H\n")
        f_out.write("#define INPUT_DATA_H\n\n")
        ...
        for input_file in hex_files:
            ...
            if len(name_parts) >= 3:
                ...
                array_name = f"FFT_input_{scenario}_{point}_Data"
            else:
                # 파일 형식이 다를 경우를 대비한 예외 처리
                array_name = f"FFT_input_{os.path.splitext(input_file)[0]}_Data"
