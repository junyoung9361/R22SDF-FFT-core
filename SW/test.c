...
int XAxiDma_Setup(u16 DeviceId)
{
    ...
    */
    ...
    // Check to ensure the DMA is not in Scatter Gather mode
    if(XAxiDma_HasSg(&AxiDma)){
        xil_printf("Device configured as SG mode \r\n");
        return XST_FAILURE;
    }

    Status = SetInterruptInit(&INST,INTR_ID, &AxiDma) ;  // Interrupt Setup
    if (Status != XST_SUCCESS)
            return XST_FAILURE ;
    ...
    */
    ...
    for(Index = 0; Index < Tries; Index ++) {
        // 1. MM2S transfer (Memory -> DMA -> FIFO)
        ...
        // 2. S2MM transfer (FIFO -> DMA -> Memory)
