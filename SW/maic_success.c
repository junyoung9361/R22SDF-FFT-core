...
static int InterruptCounter = 0; // TLAST 발생 횟수를 추적하기 위한 정적 변수

static int CheckData(void)
{
    u32 *RxPacket = RxBufferPtr; // 수신 버퍼 시작 주소
    int Index = 0;
    int Error = 0;
    double sig_pwr = 0, err_pwr = 0; // MSE 계산을 위한 전력 변수

    /* 1. 인터럽트 상태 확인 및 Ack (필수) */
    // DMA 수신 채널의 인터럽트를 인정하여 다음 인터럽트가 발생할 수 있게 함
    XAxiDma_IntrAckIrq(&AxiDma, XAXIDMA_IRQ_IOC_MASK, XAXIDMA_DEVICE_TO_DMA);

    /* 2. 인터럽트 발생 횟수 카운트 */
    InterruptCounter++;
    xil_printf("Enter Interrupt (%d/%d)\r\n", InterruptCounter, NUMBER_OF_TRANSFERS);

    /* 3. TLAST 횟수가 NUMBER_OF_TRANSFERS에 도달할 때까지 분석 대기 */
    if (InterruptCounter < NUMBER_OF_TRANSFERS) {
        return XST_SUCCESS; // 아직 모든 데이터가 차지 않았으므로 함수 종료
    }

    /* 4. 모든 전송 완료 후 분석 시작 */
    xil_printf("All Transfers Completed. Starting Error Analysis...\r\n");
    InterruptCounter = 0; // 다음 테스트를 위해 카운터 초기화

    /* 5. 데이터 캐시 무효화 (Invalidate) */
    // CPU가 DRAM에 써진 최신 데이터를 읽도록 캐시를 무효화함
    ...
    // 전체 전송된 샘플(current_point * NUMBER_OF_TRANSFERS)에 대해 전수 조사
    for(Index = 0; Index < (current_point * NUMBER_OF_TRANSFERS); Index++) {
        u32 hw_val = RxPacket[Index];
        u32 gold_val = current_gold_data[Index % current_point]; // Gold 데이터는 N포인트 단위로 반복 비교

        // 16비트 Signed 정수(실수/허수) 분리 및 복원
        ...
        // MSE(평균 제곱 오차) 계산을 위한 차이값 누적
        double re_diff = (double)gd_re - hw_re;
        double im_diff = (double)gd_im - hw_im;
        err_pwr += (re_diff * re_diff + im_diff * im_diff);
        sig_pwr += ((double)gd_re * gd_re + (double)gd_im * gd_im);

        // 하드웨어 값과 Golden 데이터 값 비교
        if (hw_val != gold_val) {
            // 값이 다를 경우 인덱스와 상세 값 출력
...
//    if (sig_pwr > 0) {
//        // SQNR/MSE dB 계산 수식 적용
//        double mse_db = 10.0 * log10(sig_pwr / err_pwr);
//        xil_printf(">> Measured SQNR: %.2f dB\r\n", mse_db);
//    }
